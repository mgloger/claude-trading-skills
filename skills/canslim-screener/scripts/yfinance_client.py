...
class YFinanceClient:
        ...
        try:
            ...
            for symbol in missing:
                try:
                    ...
                    current_volume = int(volumes.iloc[-1]) if not volumes.empty else 0
    ...
    def calculate_ema(self, prices: list[float], period: int) -> float:
        """Calculate EMA from a list of prices (most-recent-first)."""
        if not prices:
            return 0.0
    ...
    # ------------------------------------------------------------------
    # Fundamental Data (CANSLIM extensions)
    # ------------------------------------------------------------------
    ...
    def get_income_statement(
        ...
        Returns a list of dicts (most-recent first) with keys:
            date, eps, epsdiluted, revenue
        matching the fields expected by earnings_calculator.py.
        """
        ...
        try:
            ...
            for col in fin.columns[:limit]:
                ...
                # EPS: try multiple row names
                eps_val = None
                for row_name in ("Diluted EPS", "Basic EPS", "EPS"):
            ...
            print(f"WARN: get_income_statement({symbol}): {e}", file=sys.stderr)
            return None

    def get_profile(self, symbol: str) -> Optional[list[dict]]:
        """Fetch company profile via yfinance.

        Returns [{"symbol", "companyName", "sector", "industry", "mktCap", ...}]
        matching FMP's profile endpoint format.
        """
        ...
        try:
            ...
            print(f"WARN: get_profile({symbol}): {e}", file=sys.stderr)
            return None

    def get_institutional_holders(self, symbol: str) -> Optional[list[dict]]:
        """Fetch institutional holder data via yfinance.

        Returns list of dicts with keys: holder, shares, dateReported, change
        matching FMP's institutional-holder endpoint format.
        """
        ...
        try:
            ...
            for _, row in holders_df.iterrows():
                date_val = row.get("Date Reported") or row.get("Date")
                date_str = pd.Timestamp(date_val).strftime("%Y-%m-%d") if date_val and not pd.isna(date_val) else ""
                ...
                results.append({
                    ...
                    "change": 0,  # yfinance doesn't provide QoQ change
                    "pctHeld": pct_held,
                })
            ...
            print(f"WARN: get_institutional_holders({symbol}): {e}", file=sys.stderr)
            return None
