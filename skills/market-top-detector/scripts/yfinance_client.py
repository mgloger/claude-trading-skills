...
class YFinanceClient:
    ...
    def get_vix_term_structure(self) -> Optional[dict]:
        """Derive VIX term structure from ^VIX and ^VIX3M via yfinance."""
        try:
            ...
            close = vix_data.get("Close") if hasattr(vix_data, "get") else vix_data["Close"]
            if close is None or close.empty:
                return None

            vix_series = close.get("^VIX", pd.Series(dtype=float)).dropna()
            vix3m_series = close.get("^VIX3M", pd.Series(dtype=float)).dropna()

            if vix_series.empty:
                return None

            vix_spot = float(vix_series.iloc[-1])

            if not vix3m_series.empty:
                vix3m = float(vix3m_series.iloc[-1])
                ratio = vix3m / vix_spot if vix_spot > 0 else 1.0
                if ratio > 1.15:
            ...
            return {
                ...
                "vix3m": vix3m,
                "ratio": ratio,
                "classification": term_structure,
                "term_structure": term_structure,
            }
        except Exception as e:
            print(f"WARN: Could not fetch VIX term structure: {e}", file=sys.stderr)
            return None
